...
void create_and_load_bgr(AVFrame *pFrame, AVFrame *pFrameBGR, uint8_t *buffer,
    ...
    av_image_fill_arrays(pFrameBGR->data, pFrameBGR->linesize, buffer, AV_PIX_FMT_BGR24, pFrame->width, pFrame->height, 1);
...
int decode_video(
    ...
    uint8_t* in_buffer = (uint8_t*)malloc((in_buffer_size + FF_INPUT_BUFFER_PADDING_SIZE)*sizeof(uint8_t));
